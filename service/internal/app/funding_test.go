...
func TestFundIsOnlyForTheParties(t *testing.T) {
	...
	if _, err := f.app.Fund(t.Context(), f.contract.ID, carol); !errors.Is(err, ErrNotAllowed) {
...
func TestFundRefusesBeforeMovingWhenASideCannotPay(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			f := newFixture(t)
			f.accepted(t)
			f.stack.balances[f.bob] = tc.balance
...
func TestFundingIsSafeToRepeat(t *testing.T) {
	...
	f.exits.fail = errors.New("the database went away")
	...
	if f.stack.fundCalls != 1 {
		t.Errorf("the funding transaction was submitted %d times, want 1", f.stack.fundCalls)
	}
}
